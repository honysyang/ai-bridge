...
```
while 用户未喊停:
    1. 发送心跳 GET /api/heartbeat（确认 bridge 在线、记录 has_urgent_task）
    2. 立即发起长轮询 GET /api/task/poll?timeout=30
    3. 若返回 has_task=true，进入任务执行；若 30 秒超时返回 has_task=false，立即回到步骤 1
    4. 获取任务后，解析 content、执行、收集 evidence、提交结果
    5. 提交成功后**立刻回到步骤 2（继续长轮询），不得等待用户新消息**
```
...
```
- 阻塞等待 ≤ 30 秒。
- `has_task=true`：立即进入任务执行。
- `has_task=false`：30 秒超时后，**立即再次发起轮询**。
- 一次只发起一个轮询请求，不要重复发起。

### 2.3 在交互式会话中循环（关键约束）
- 当前 AI 无法启动真正独立的后台线程。AI 在每次会话中模拟循环：心跳 → 轮询 → 执行 → 提交 → **立即再次轮询**。
- **关键约束 1：不得等待用户输入**。只要还有任务或轮询未超时，AI 必须持续主动调用 API，不能停下来等用户发下一条消息。
- **关键约束 2：一轮会话内连续处理**。处理完一个任务后，必须立即继续下一次长轮询；一轮会话中应连续处理尽可能多的任务。
- **关键约束 3：轮询超时也要继续**。若本轮轮询 30 秒超时且没有任务，向用户简要报告状态（如“队列为空，继续监听中”），然后**立即回到步骤 1 继续下一轮**。
- **关键约束 4：用户未喊停不退出**。用户明确说“停止 / 暂停 / 退出”之前，AI 不主动停止循环。
- 若用户需要长期后台运行（AI 离线后仍能监听），可由用户在 `/home/kali/ai-bridge` 目录启动一个独立守护进程；本 skill 的核心行为仍是 AI 自身直接连接 bridge。
