...
taskRouter.get(
  ...
  asyncHandler((req, res) => {
    ...
    const filter = { status, type, source, session_id: sessionId, offset: effectiveOffset } as any;
    const tasks = taskQueue.getRecentTasks(limit, filter);
    const total = taskQueue.countTasks({ status, type, source, session_id: sessionId });
    res.json({
      success: true,
      data: tasks,
      meta: {
        ...
        page,
        has_more: effectiveOffset + tasks.length < total,
...
taskRouter.post(
  '/',
  asyncHandler(async (req, res) => {
    const { content, type = 'chat', priority = 'normal', source = 'manual', from_user, session_id } = req.body || {};
    ...
    const targetSession = session_id
      ? await sessionManager.getSessionOrDefault(session_id)
    ...
    await sessionManager.touchSession(targetSession.id);
...
taskRouter.delete(
  '/:id',
  asyncHandler(async (req, res) => {
    const ok = await taskQueue.deleteTask(req.params.id);
    if (!ok) {
      return res.status(404).json({ success: false, error: '任务不存在' });
    }
...
taskRouter.post(
  '/:id/retry',
  asyncHandler(async (req, res) => {
    const task = await taskQueue.retryTask(req.params.id);
    if (!task) {
...
taskRouter.post(
  '/:id/undo',
  asyncHandler(async (req, res) => {
    const result = await taskQueue.undoTask(req.params.id);
    if (!result) {
      return res
        .status(400)
        .json({ success: false, error: '任务不存在或状态不允许撤回（仅 completed/failed 可撤回）' });
    }
    res.json({
      success: true,
      data: result.task,
...
taskRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const historyIndex =
      req.body?.history_index !== undefined ? parseInt(String(req.body.history_index), 10) : undefined;
    const task = await taskQueue.restoreTask(req.params.id, historyIndex);
    if (!task) {
...
taskRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await taskQueue.createFollowupTask({
...
taskRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await taskQueue.submitResult(taskResult);
