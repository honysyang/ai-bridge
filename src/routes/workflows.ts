...
// 列表
workflowRouter.get('/', asyncHandler((_req, res) => {
  res.json({ success: true, data: workflowStore.list() });
}));

// 追加演示
workflowRouter.post('/seed-demo', asyncHandler((_req, res) => {
...
// 详情
workflowRouter.get('/:id', asyncHandler((req, res) => {
...
// 创建
workflowRouter.post('/', asyncHandler((req, res) => {
...
// 更新
workflowRouter.patch('/:id', asyncHandler((req, res) => {
...
// 删除
workflowRouter.delete('/:id', asyncHandler((req, res) => {
...
workflowRouter.post('/:id/execute', asyncHandler((req, res) => {
  const wf = workflowStore.get(req.params.id);
  if (!wf) {
    return res.status(404).json({ success: false, error: '工作流不存在' });
  }
