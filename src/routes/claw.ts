...
clawRouter.get(
  ...
  asyncHandler((_req, res) => {
    ...
    const queue = bridge && typeof bridge.getQueueStatus === 'function' ? bridge.getQueueStatus() : [];
...
clawRouter.get(
  ...
  asyncHandler((_req, res) => {
    const bridge = (clawManager as any).bridge;
    const status = bridge && typeof bridge.getSummaryStatus === 'function' ? bridge.getSummaryStatus() : [];
...
clawRouter.get(
  ...
  asyncHandler((_req, res) => {
    ...
    const allSet = out['ILINK_BOT_TOKEN'] && out['ILINK_BOT_ID'] && out['ILINK_USER_ID'];
    out['configured'] = allSet ? 'true' : 'partial';
