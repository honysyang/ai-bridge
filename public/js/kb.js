...
(function (global) {
  ...
  function bindKBEvents() {
    ...
    if (search) {
      ...
      search.addEventListener('input', (e) => {
        ...
        timer = setTimeout(() => {
          ...
          state.kbSearchKeyword = kbState.searchKeyword;
          loadKB();
    ...
    if (catFilter) {
      catFilter.addEventListener('change', (e) => {
        ...
        state.kbCategoryId = state.currentKbCategoryId;
        loadKB();
      });
    }
  ...
  async function loadKB(append = false) {
    if (!append) showSkeletonKB();
    try {
      const params = new URLSearchParams();
      params.set('limit', String(state.kbPagination.limit));
      const offset = append ? state.kbPagination.offset + state.kbPagination.limit : 0;
      ...
      state.kbPagination.offset = offset;
      state.kbPagination.total = meta?.total_count || 0;
      state.kbPagination.hasMore = meta?.has_more || false;
  ...
  function loadMoreKBItems() {
    if (state.kbPagination.hasMore) loadKB(true);
  }

  function updateKBStats() {
    const el = document.getElementById('kb-stats');
    const total = state.kbPagination?.total ?? state.kbItems.length;
    if (el) el.textContent = `${state.kbCategories.length} 分类 / ${total} 条目`;
  }
  ...
  function renderKBItems() {
    ...
    if (count) count.textContent = `${state.kbPagination?.total || items.length} 条`;
    ...
    // v5.5.6: KB 分页加载更多
    if (state.kbPagination.hasMore) {
      list.insertAdjacentHTML(
        'beforeend',
        `
        <button class="btn btn-ghost load-more" id="load-more-kb" style="grid-column:1/-1;width:100%;margin-top:8px;">
          加载更多 (${state.kbPagination.offset + state.kbItems.length} / ${state.kbPagination.total})
        </button>`
      );
      document.getElementById('load-more-kb')?.addEventListener('click', loadMoreKBItems);
    }
  }
  ...
  function fillChatWithKBItem(itemOrId) {
    const item = typeof itemOrId === 'string' ? state.kbItems.find((i) => i.id === itemOrId) : itemOrId;
    if (!item) return;
  ...
  // ======== 知识图谱 ========
  const KB_CATEGORY_COLORS = ['#667eea', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16'];
  ...
  function renderKBGraph() {
    ...
    if (typeof cytoscape === 'undefined') {
      container.innerHTML =
        '<div style="padding:40px;text-align:center;color:#94a3b8">⚠️ Cytoscape.js 未加载（CDN 失败？）</div>';
      return;
    }
    ...
    state.cy.on('tap', 'edge', async (evt) => {
  ...
  async function seedKBDemo() {
    const ok = await global.Core.openConfirm({
      title: '加载示例知识库',
      message: '将追加 6 个分类 / 25+ 条目 / 20+ 跨分类关联。\n已存在的数据会自动跳过。\n继续？',
      confirmText: '继续'
    });
    if (!ok) return;
